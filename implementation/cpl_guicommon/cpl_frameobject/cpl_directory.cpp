...
//-----------------------------------------------------------------------------
//
cpl_Directory* cpl_Directory::New() {
    return new cpl_Directory;
}

//-----------------------------------------------------------------------------
void cpl_Directory::CleanUpFilesAndPath() {
...
//-----------------------------------------------------------------------------
int cpl_Directory::Open(const char* name) {
...
//-----------------------------------------------------------------------------
const char* cpl_Directory::GetCurrentWorkingDirectory(char* buf, unsigned int len) {
...
//-----------------------------------------------------------------------------
const char* cpl_Directory::GetFile(int index) {
...
//-----------------------------------------------------------------------------
int cpl_Directory::FileIsDirectory(const char * name) {
    struct stat fs;
    if (stat(name, & fs) == 0) {
#if _WIN32
...
//-----------------------------------------------------------------------------
cpl_Directory::cpl_Directory() : Path(0), Files(0), NumberOfFiles(0) {
}

//-----------------------------------------------------------------------------
cpl_Directory::~cpl_Directory() {
    this->CleanUpFilesAndPath();
}
